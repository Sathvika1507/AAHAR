...
public class deleteProfileActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.oldpasswordLayout);
    ...
    private void reAuthenticateUser(FirebaseUser firebaseUser)  {
        buttonReAuthenticate.setOnClickListener(view -> {
            userPwd= Objects.requireNonNull(editTextUserPwd.getText()).toString();
    ...
    private void deleteUserData(FirebaseUser firebaseUser) {
        ...
        usersRef.child(firebaseUser.getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                if (dataSnapshot.exists()) {
                    ...
                    removeFoodPinAndDonationIdMapping(a);
                } else { //exist not
                    //do nothing
                }
            }
        ...
        databaseReference.child(firebaseUser.getUid()).removeValue().addOnSuccessListener(unused -> Log.d(TAG, "onSuccess: User data1 Deleted")).addOnFailureListener(e -> {
            Log.d(TAG, Objects.requireNonNull(e.getMessage()));
            Toast.makeText(deleteProfileActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
        });
        //deleting History
        databaseReferenceThree.child(firebaseUser.getUid()).removeValue().addOnSuccessListener(unused -> {
    ...
    private void removeFoodPinAndDonationIdMapping(String a) {
        ...
        databaseReferenceTwo.child(firebaseUser.getUid()).removeValue().addOnSuccessListener(unused -> Log.d(TAG, "onSuccess: User data2 Deleted")).addOnFailureListener(e -> {
            Log.d(TAG, Objects.requireNonNull(e.getMessage()));
            Toast.makeText(deleteProfileActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
        });
        //delete FoodMap pin
        databaseReferenceOne.child(a).removeValue().addOnSuccessListener(unused -> Log.d(TAG, "onSuccess: User data2 Deleted")).addOnFailureListener(e -> {
            Log.d(TAG, Objects.requireNonNull(e.getMessage()));
            Toast.makeText(deleteProfileActivity.this, e.getMessage(), Toast.LENGTH_SHORT).show();
        });
    }
